...
struct MANGOS_DLL_DECL boss_midnightAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            Phase = 2;
            Creature *pAttumen = DoSpawnCreature(SUMMON_ATTUMEN, 0, 0, 0, 0, TEMPSUMMON_DEAD_DESPAWN, 500000);
            if(pAttumen)
            {
...
struct MANGOS_DLL_DECL boss_attumenAI : public ScriptedAI
{
    ...
    void JustDied(Unit *victim)
    {
        ...
        if(pMidnight)
        {
            pMidnight->DealDamage(pMidnight, pMidnight->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_NORMAL, NULL, 0, false);
        }
    }
