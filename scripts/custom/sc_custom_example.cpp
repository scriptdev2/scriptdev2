...
struct MANGOS_DLL_DECL custom_exampleAI : public ScriptedAI
{
    ...
    void AttackStart(Unit *who)
    {
        ...
        if (m_creature->getVictim() == NULL && who->isTargetableForAttack() && who != m_creature)
        {
            ...
            //Say some stuff
            DoSay("Let the games begin.",LANG_UNIVERSAL);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!m_creature->getVictim())
        {
            ...
            if (Say_Timer < diff)
            {
                ...
                {
                case 0:
                    DoSay("I see endless suffering. I see torment. I see rage. I see everything.",LANG_UNIVERSAL);
                ...
                case 1:
                    DoSay("Muahahahaha",LANG_UNIVERSAL);
                    DoPlaySoundToSet(m_creature,8818); 
                    break;

                case 2:
                    DoSay("These mortal infedels my lord, they have invaded your sanctum and seek to steal your secrets.",LANG_UNIVERSAL);
                    DoPlaySoundToSet(m_creature,8041); 
                    break;

                case 3:
                    DoSay("You are already dead.",LANG_UNIVERSAL);
                    DoPlaySoundToSet(m_creature,8581);
                    break;

                case 4:
                    DoSay("Where to go? What to do? So many choices that all end in pain, end in death.",LANG_UNIVERSAL);
        ...
        if( m_creature->getVictim() && m_creature->isAlive())
        {
            ...
            if (Phase > 1)
                if (Beserk_Timer < diff)
                {
                    ...
                    DoSay("I sentance you to death!",LANG_UNIVERSAL);
            ...
            if (Phase == 1)
                if (Phase_Timer < diff)
                {
                    //Go to next phase
                    Phase++;
                    DoSay("The suffering has just begun!",LANG_UNIVERSAL);
...
{
    player->ADD_GOSSIP_ITEM( 0, "I'm looking for a fight"        , GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
bool ReciveEmote_custom_example(Player *player, Creature *_Creature, uint32 emote)
{
    ...
    if (emote == EMOTE_STATE_DANCE)
        ((custom_exampleAI&)_Creature->AI()).DoSay("I always thought I was a good dancer",LANG_UNIVERSAL);

    if (emote == EMOTE_ONESHOT_SALUTE)
        ((custom_exampleAI&)_Creature->AI()).DoSay("Move out Soldier!",LANG_UNIVERSAL);
